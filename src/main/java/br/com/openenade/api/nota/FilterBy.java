...
public class FilterBy {
	...
	public FilterBy filterByRegiao(String regiao) {
		...
		if (regiao != null) {
			this.notas = this.notas.stream().filter(nota -> nota.getInfo().getUniversidade().getCampus().getEstado()
	...
	public FilterBy filterByCategAdmin(CategoriaAdmin catAdmin) {
		...
		if (catAdmin != null) {
			this.notas = this.notas.stream()
					.filter(nota -> nota.getInfo().getUniversidade().getCategoriaAdmin().equals(catAdmin))
					.collect(Collectors.toList());
		}
		return this;
	}
	...
	public FilterBy filterByCodigoIES(Long codigoIES) {
		...
		if (codigoIES != null) {
			this.notas = this.notas.stream()
					.filter(nota -> (nota.getInfo().getUniversidade().getCodigoIES().equals(codigoIES)))
					.collect(Collectors.toList());
		}
		return this;
	}
	...
	public FilterBy filterByAno(Integer ano) {

		if (ano != null) {
			this.notas = this.notas.stream().filter(nota -> nota.getInfo().getAno().getAno().equals(ano))
					.collect(Collectors.toList());
		}
		return this;
	}
